# ====================================================
# = This is based on HotCocoa application_builder.rb =
# ====================================================
...
module HotCocoa
  ...
  class ApplicationBuilder
    ...
    def add_resource_path(resource_file_pattern)
      (Dir.glob(resource_file_pattern).reject {|f| f == @info_plist}).each do |resource_file|
    ...
    private
      ...
      def copy_resources
        resources.each do |resource|
          ...
          FileUtils.mkdir_p(File.dirname(destination)) unless File.exist?(File.dirname(destination))
          # Compile XIB files. All the rest just copy
          if resource =~ /\.xib$/
            destination = destination.sub(/\.xib$/, '.nib')
            puts `/Developer/usr/bin/ibtool --errors --warnings --notices --output-format human-readable-text --compile #{destination} #{resource}`
          else
      ...
      def write_info_plist_file
        File.copy(@info_plist, info_plist_file, verbose:false)
      end
